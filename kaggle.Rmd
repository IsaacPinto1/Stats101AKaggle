...
m0 <- lm(as.formula(
  paste("LogSalePrice ~ I(log(TotalSF))+I(LogTotalBsmtSF^2) +LogTotalBsmtSF+ I(LogX1stFlrSF^2) + OverallQualSq  + I(sqrt(SentimentScore+2)) + GarageCarsClamped + Log_LotArea")
), data = train) # 0.906, 10 betas
 
m0 <- lm(as.formula(
  paste("LogSalePrice ~ I(log(TotalSF)) +LogTotalBsmtSF+ LogX1stFlrSF + OverallQualSq  + I(sqrt(SentimentScore+2)) + GarageCarsClamped + Log_LotArea")
), data = train) # 0.906, 10 betas

m0 <- lm(as.formula(
  paste("LogSalePrice ~ TotalSF+I(LogTotalBsmtSF^2) + I(sqrt(SentimentScore2+1)) + LogTotalBsmtSF+ LogX1stFlrSF + OverallQualSq + SentimentScore + GarageCarsClamped + Log_LotArea")
), data = train) # 0.90875, 10 betas
...
engineer_features_basic_location <- function(df) {
  ...
  sentiment_scores2 <- df$House_Review_Text %>%
  get_sentences() %>%
  sentiment_by(by = NULL) %>%    # NULL = score each text as one unit
  pull(ave_sentiment)

  df %>%
    mutate(
      SentimentScore = sentiment_scores,
      SentimentScore2 = sentiment_scores2,
      ...
      GarageCarsClamped = pmin(GarageCars, 4),
      ...
      Location2  = factor(case_when(
        Neighborhood %in% c("NAmes", "CollgCr", "OldTown", "Somerst", "Edwards", "Gilbert", "NridgHt", "Sawyer", "SawyerW", "NWAmes", "Mitchel", "BrkSide", "Crawfor") ~ 1,
        TRUE ~ 2
      )),
      HasSecondFloor = factor(case_when(
        X2ndFlrSF > 0 ~ 1,
        TRUE ~ 2
      )),
...
model_initial <- lm(as.formula(
  paste("LogSalePrice ~ I(sqrt(TotalSF))+I(LogTotalBsmtSF^2) +LogTotalBsmtSF+ I(LogX1stFlrSF^2) + OverallQualSq*Location + I(sqrt(SentimentScore+2)) + GarageCars + Log_LotArea")
), data = train) # 0.857, 10 betas

model_initial <- lm(as.formula(
  paste("LogSalePrice ~ TotalSF+I(LogTotalBsmtSF^2) +LogTotalBsmtSF+ I(LogX1stFlrSF^2) + OverallQualSq*Location + I(sqrt(SentimentScore+2)) + GarageCars + Log_LotArea")
), data = train) # 0.906, 10 betas

model_initial <- lm(as.formula(
  paste("LogSalePrice ~ TotalSF+I(LogTotalBsmtSF^2) +LogTotalBsmtSF+ I(LogX1stFlrSF^2) + OverallQualSq*Location + I(sqrt(SentimentScore+2)) + GarageCars + Log_LotArea + RoadRail")
), data = train) # 0.90634, 11 betas
...
model_initial <- lm(as.formula(
  paste("LogSalePrice ~ TotalSF+I(LogTotalBsmtSF^2) +LogTotalBsmtSF+ I(LogX1stFlrSF^2) + OverallQualSq*Location + I(sqrt(SentimentScore+2)) + GarageCars + Log_LotArea")
), data = train) # 0.90620, 10 betas, 0.90210 without bad leverage

model_initial <- lm(as.formula(
  paste("LogSalePrice ~ TotalSF+I(LogTotalBsmtSF^2) +LogTotalBsmtSF+ I(LogX1stFlrSF^2) + OverallQualSq*Location + I(sqrt(SentimentScore+2)) + GarageCarsClamped + Log_LotArea")
), data = train) # 0.90652, 10 betas

model_initial <- lm(as.formula(
  paste("LogSalePrice ~ TotalSF+I(LogTotalBsmtSF^2) +LogTotalBsmtSF+ LogX1stFlrSF + OverallQualSq*Location + I(sqrt(SentimentScore+2)) + GarageCarsClamped + Log_LotArea")
), data = train) # 0.90875, 10 betas

model_initial <- lm(as.formula(
  paste("LogSalePrice ~ TotalSF + I(LogTotalBsmtSF^2) + LogX1stFlrSF + OverallQualSq*Location + I(sqrt(SentimentScore+2)) + GarageCarsClamped + Log_LotArea")
), data = train) # 0.91549, 9 betas

model_initial <- lm(as.formula(
  paste("LogSalePrice ~ TotalSF + I(LogTotalBsmtSF^2) + LogX1stFlrSF+ OverallQualSq*Location + I(sqrt(SentimentScore+2)) + GarageCars + Log_LotArea")
), data = train) 
...
print(model_cv$results)  # gives you mean R² across folds
